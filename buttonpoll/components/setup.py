...
class StartSetupView(discord.ui.View):
    """Initial view with a button to open the poll setup modal."""
...
class SetupModal(ui.Modal, title="Poll setup"):
    """Modal for entering poll question, description, duration, and options."""
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        str_options = [o.strip() for o in str_options if o.strip()]
        ...
        for str_option in str_options:
            ...
            options.append(PollOption(str_option, discord.ButtonStyle.primary))
...
class SetupYesNoView(discord.ui.View):
    """View with select menus for poll configuration options, then a submit button."""
    ...
    @discord.ui.select(
        ...
        options=[
            SelectOption(
                ...
                description="Send a new message with results when the poll is over.",
                value="send_new",
            ),
            SelectOption(
                label="Edit original message",
                description="Edit the original poll message with results.",
                value="edit_old",
            ),
        ],
    )
    async def sel_send_msg_when_over(
        self, interaction: discord.Interaction, select: discord.ui.Select
    ):
        self.send_msg_when_over = select.values[0] == "send_new"
        await interaction.response.defer()
