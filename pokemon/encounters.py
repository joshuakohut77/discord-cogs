...
class EncountersMixin(MixinMeta):
    ...
    async def on_wild_battle_auto(self, interaction: discord.Interaction):
        ...
        while player_pokemon_index < len(alive_party) and enemy_pokemon_index < len(enemy_pokemon_list):
            ...
            # Fight this matchup - full auto battle
            ...
            if result.get('result') == 'victory':
                ...
                # Defeat OR draw - treat as player loss for this matchup
...
# =============================================================================
    ...
    async def on_wild_battle_throw_ball(self, interaction: discord.Interaction):
        ...
        if trainer.statuscode == 420:
            ...
            try:
                success_embed.set_thumbnail(url=battle_state.wild_pokemon.frontSpriteURL)
...
# =============================================================================
    ...
    async def on_pc_page_prev_click(self, interaction: discord.Interaction):
        """Handle PC page previous button"""
        user = interaction.user
        await interaction.response.defer()
        ...
        bag_state = self.__bag_states[str(user.id)]
        bag_state.pc_page = max(0, bag_state.pc_page - 1)
        ...
        selected_trainer_id = bag_state.pc_selected_pokemon_id or (str(pc_list[0].trainerId) if pc_list else None)

        embed, view = self._build_pc_view(user, bag_state, pc_list, selected_trainer_id)
        await interaction.message.edit(embed=embed, view=view)

    async def on_pc_page_next_click(self, interaction: discord.Interaction):
        """Handle PC page next button"""
        user = interaction.user
        await interaction.response.defer()
    ...
    def _build_pc_view(self, user, bag_state, pc_list, selected_trainer_id):
        """
        Build the PC embed and view with pagination support.
        Returns (embed, view). Handles the 25-option select menu limit.
        """
        from discord.ui import Select
        from discord import SelectOption
        from .functions import createStatsEmbed
        ...
        # Clamp page
        if bag_state.pc_page < 0:
            bag_state.pc_page = 0
        if bag_state.pc_page >= total_pages:
            bag_state.pc_page = total_pages - 1

        page = bag_state.pc_page
        page_start = page * PAGE_SIZE
        page_end = min(page_start + PAGE_SIZE, total)
        page_list = pc_list[page_start:page_end]

        # If selected Pokemon isn't on this page, default to first on page
        page_ids = [str(p.trainerId) for p in page_list]
        if selected_trainer_id not in page_ids:
            selected_trainer_id = page_ids[0] if page_ids else None
            bag_state.pc_selected_pokemon_id = selected_trainer_id

        # Find selected Pokemon
        selected_pokemon = None
        for poke in page_list:
            if str(poke.trainerId) == selected_trainer_id:
                selected_pokemon = poke
                break
        if selected_pokemon is None and page_list:
            selected_pokemon = page_list[0]
            selected_trainer_id = str(selected_pokemon.trainerId)
            bag_state.pc_selected_pokemon_id = selected_trainer_id

        # Reload selected Pokemon for full data
        if selected_pokemon:
            selected_pokemon.load(pokemonId=selected_pokemon.trainerId)
        ...
        if selected_pokemon:
            ...
            embed.title = f"💾 PC - {embed.title}{page_label}"
        else:
            embed = discord.Embed(
                title="💾 PC Storage",
                description="No Pokemon found on this page.",
                color=discord.Color.blue()
            )

        if total_pages > 1:
            embed.set_footer(text=f"Showing {page_start + 1}-{page_end} of {total} Pokemon")

        # Build view
        view = View()

        # ROW 0: Pokemon selector
        select = Select(placeholder=f"Choose a Pokemon (Page {page + 1}/{total_pages})", custom_id='pc_pokemon_select', row=0)
        for poke in page_list:
            poke.load(pokemonId=poke.trainerId)
        ...
        withdraw_btn = Button(style=ButtonStyle.green, label="⬆️ Withdraw", custom_id='bag_pc_withdraw', row=1)
        withdraw_btn.callback = self.on_bag_pc_withdraw_click
        view.add_item(withdraw_btn)

        # Page navigation buttons (only if multiple pages) - also on row 1
        if total_pages > 1:
            prev_btn = Button(
                style=ButtonStyle.gray,
                label="◀ Prev",
                custom_id='pc_page_prev',
                row=1,
                disabled=(page == 0)
            )
            prev_btn.callback = self.on_pc_page_prev_click
            view.add_item(prev_btn)

            next_btn = Button(
                style=ButtonStyle.gray,
                label="Next ▶",
                custom_id='pc_page_next',
                row=1,
                disabled=(page >= total_pages - 1)
            )
            next_btn.callback = self.on_pc_page_next_click
            view.add_item(next_btn)
    ...
    async def on_bag_pc_withdraw_click(self, interaction: discord.Interaction):
        """Handle withdraw button in PC view"""
        user = interaction.user
        await interaction.response.defer()
    ...
    async def on_bag_pc_click(self, interaction: discord.Interaction):
        ...
        bag_state.pc_page = 0  # Reset to first page when opening PC
        ...
        # Default to first Pokemon if no selection
